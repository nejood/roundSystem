...
<head> <!-- start Head-->
<title>Smart Board Information</title>  <!-- the name of tab -->
...
<center>
<h1 class="titles" id="title1" >Smart Board Information</h1>        <!-- the page title -->
<h1 class="titles" id="title1">_____________________________</h1>

<form action="" method="post">
<!-- start form to take input from user and do action -->

<label class="labels" id="label1">smart board model: </label> <!-- the label of text box to help user to now what the requierd -->

 <input class="textbox" id="textbox" type="text" name="smart_board_model">  <!-- this text box just acsept the input from email type -->
...
$smart_board_model=$_POST['smart_board_model'];
...
//parameters: connection variable and sql query
 ...
 if (isset($row[0])) {//there is a device

//update the chosen device's data from '' to the entered data
$up="UPDATE deviceinclass SET smart_board_model ='$smart_board_model' where device_pk='$row[0]'";
...
{
    echo "<script>
alert('The smart board was added successfully .');
</script>";  
...
else{ //there is no device in the class


//query to insert the class into the deviceinclass table and the entered device info
$in="INSERT INTO deviceinclass (class_pk,smart_board_model)
     VALUES ('$class_pk','$smart_board_model')";
