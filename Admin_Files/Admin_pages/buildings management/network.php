...
<head> <!-- start Head-->
<title>Network Information</title>  <!-- the name of tab -->
</head> <!-- end Head-->
...
<center>
<h1 class="titles" id="title1" >Network Information</h1>        <!-- the page title -->
<h1 class="titles" id="title1">_____________________________</h1>

<form action="" method="post">
<!-- start form to take input from user and do action -->

<label class="labels" id="label1">network port: </label> <!-- the label of text box to help user to now what the requierd -->

 <input class="textbox" id="textbox" type="text" name="network_port">  <!-- this text box just acsept the input from email type -->
...
$network_port=$_POST['network_port'];
...
$up="UPDATE deviceinclass SET network_port ='$network_port' where device_pk='$row[0]'";
$check=mysqli_query($conn,$up);

//when the upadate query done successfully 
if(isset($check))
{echo "<script>
alert('The network port was added successfully .');
</script>";
...
$in="INSERT INTO deviceinclass (class_pk,network_port)
     VALUES ('$class_pk','$network_port')";
